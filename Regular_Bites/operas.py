...
def operas_both_at_premiere(guest, composer):
    ...
    """
    if composer not in composers.keys() or guest not in composers.keys():
        raise ValueError

    guest_dday = _get_date(composers[guest].died)
    composer_dday = _get_date(composers[composer].died)
    guest_bday = _get_date(composers[guest].born)

    for opera in operas:
        opera_date = _get_date(opera.date)
        if (opera_date < composer_dday and guest_bday < opera_date < guest_dday) and (opera.author == composer):
            yield opera
